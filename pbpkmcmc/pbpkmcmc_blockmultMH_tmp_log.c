...
int main(int argc,char *argv[])
{
  ...
  if (color_split!=MPI_UNDEFINED) { // lasse die überflüssigen Prozessoren außer acht.
    ...
    */
    if (chain_rank==masterrank) {
      Ith(thetaCurr,1) = log(0.4) + (world_rank - (int)round(world_size/2)) * 0.1; // sigma der Likelihood-Funktion
    } else {
      ...
      for (ctr = 0; ctr < blocksPerRank; ctr++) {
        for (kk = 1; kk <= 21; kk++) {
          ...
          Ith(thetaCurr,ctr*NPARSTUDY+kk) = log(Ith(posteriorZurlindenMean,kk)) + (world_rank - (int)round(world_size/2)) * log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);
        }
