...
//  if (check_retval(&retval, "CVodeRootInit", 1)) return(1);
  ...
  printf("waitbar: ");
  fprintf(fileTimes, "iter\t");
  for (studyblock = 0; studyblock < NSTUDIES; studyblock++) {
    fprintf(fileTimes, "ODEfail%d\tODEsolve%d\t", studyblock,studyblock);
  }
  fprintf(fileTimes, "posterior\tswapbc\tswapbar\tswappa\tswapwait\tswap\tcpuOverall\tnIterSund\tcpuPerSundials\n");
  ...
  for (iterJ = 0; iterJ < iterAll; iterJ++) {
    ...
    */
    ...
    // sigmaLL würfeln
    Ith(thetaCan,NPARALL) = Ith(thetaCurr,NPARALL)+gsl_ran_gaussian(gslrng,1e-3); // würfele sigmaLL mittels Random Walk
    ...
    for (studyblock = 0; studyblock < NSTUDIES; studyblock++) {
      ...
      while(1) {
        ...
        if (check_retval(&retval, "CVode", 1)) {
          ...
          endtime_ODE_fail = MPI_Wtime(); fprintf(fileTimes, "%10.6e\t", endtime_ODE_fail - endtime_ODE_reinit); //fprintf(fileTimes, "\nposteriorODEFailed\t%d\t%10.6e", studyblock, endtime_ODE_fail - endtime_ODE_reinit);
      ...
      if (nAcceptBool!=-2) {
        fprintf(fileTimes, "%10.6e\t", 0.0);
      }
      else {
        nIterSundials_fail += nIterSundials;
      }
      endtime_ODE = MPI_Wtime(); cputime_ODE_solve = endtime_ODE - endtime_ODE_reinit; fprintf(fileTimes, "%10.6e\t", cputime_ODE_solve); // fprintf(fileTimes, "\nposteriorODEsolve\t%d\t%10.6e", studyblock, cputime_ODE_solve);
      if (iterJ>0) {
    ...
    cputime_posterior = endtime_posterior-endtime_proposal; fprintf(fileTimes, "%10.6e\t", cputime_posterior); // fprintf(fileTimes, "\nposterior\t%10.6e", cputime_posterior);
    ...
    if (iterJ>0) { // bei j==0 wird noch der theta_Anfangswertvektor ausgewertet
      ...
      if ((world_size>1)&&(swaps>1)) {
        if (world_rank==RANKMASTER) {
          ...
          endtime_swap_bcast = MPI_Wtime(); cputime_swap_bcast = endtime_swap_bcast-endtime_swap_toss; fprintf(fileTimes, "%10.6e\t", cputime_swap_bcast); // fprintf(fileTimes, "\nswapbcast\t%10.6e", cputime_swap_bcast);
          ...
          endtime_swap_bcast = MPI_Wtime(); cputime_swap_bcast = endtime_swap_bcast-endtime_accrej; fprintf(fileTimes, "%10.6e\t", cputime_swap_bcast); // fprintf(fileTimes, "\nswapbcast\t%10.6e", cputime_swap_bcast);
        }

        // endtime_swap_bcast = MPI_Wtime(); cputime_swap_bcast = endtime_swap_bcast-endtime_accrej; fprintf(fileTimes, "\nswapbcast\t%10.6e", cputime_swap_bcast);
        MPI_Barrier(MPI_COMM_WORLD);
        endtime_swap_barrier = MPI_Wtime(); cputime_swap_barrier = endtime_swap_barrier-endtime_swap_bcast; fprintf(fileTimes, "%10.6e\t", cputime_swap_barrier); // fprintf(fileTimes, "\nswapbarrier\t%10.6e", cputime_swap_barrier);
        ...
        if (world_rank!=swapPartner) {
          ...
          endtime_swap_partner = MPI_Wtime(); cputime_swap_partner = endtime_swap_partner-endtime_swap_barrier; fprintf(fileTimes, "%10.6e\t", cputime_swap_partner); // fprintf(fileTimes, "\nswappartner\t%10.6e", cputime_swap_partner);

          MPI_Wait(&request_swap_theta, &status_swap_theta);
          MPI_Wait(&request_swap_logPosterior, &status_swap_logPosterior);
          endtime_swap_wait = MPI_Wtime(); cputime_swap_wait = endtime_swap_wait-endtime_swap_partner; fprintf(fileTimes, "%10.6e\t", cputime_swap_wait); // fprintf(fileTimes, "\nswapwait\t%10.6e", cputime_swap_wait);
        ...
        fprintf(fileTimes, "%10.6e\t%10.6e\t%10.6e\t%10.6e\t", 0.0, 0.0, 0.0, 0.0);
      }
      endtime_swap = MPI_Wtime(); fprintf(fileTimes, "%10.6e\t", endtime_swap - endtime_accrej); // fprintf(fileTimes, "\nswap\t%10.6e", endtime_swap - endtime_accrej);
  ...
  fprintf(fileLog, "Zeiten\t\tZeiten\t\tZeiten\t\tZeiten\t\tZeiten\t\tZeiten\t\tnIter\n");
  fprintf(fileLog, "\t\tODE\t\t\t\t\t\tKommunikation\tSundials\tSundials\n");
  fprintf(fileLog, "overall\t\tsum\t\tfail\t\tsum-fail\tswap\t\tsecPerSundIter\tall\tfail\n");
  fprintf(fileLog, "%10.4e\t%10.4e\t%10.4e\t%10.4e\t%10.4e\t%10.4e\t%ld\t%ld\n", cputime_iterAll, cputime_ODE_sum, cputime_ODE_fail, cputime_ODE_sum-cputime_ODE_fail, cputime_swap, (double) cputime_ODE_sum/nIterSundials_sum, nIterSundials_sum, nIterSundials_fail);
