...
#define SIGMA_RW RCONST(1e-1)
#define SIGMA_LL RCONST(1e-3)
...
{
  double starttime_iterAll, endtime_iterAll, starttime_iter, endtime_iter, starttime_setup, endtime_setup;
  ...
  double cputime_ODE, cputime_ODE_sum=0.0, cputime_iterAll, cputime_swap=0.0, cputime_iter=0.0, cputime_setup=0.0;
  ...
  if (argc>3) {
    ...
    seedBool = -1;
    printf("No input for boolean seed. We take a look if multiple chains are started.\n");
  }
  ...
  if (argc>4) {
    ...
    switch (multipleChainsBool) {
      case 0:
        if (seedBool==-1) {
          seedBool = 0;
          printf("\tOne chain started. Take default seed for random number generator.\n");
        }
        break;
      case 1:
        if (seedBool==-1) {
          seedBool = 1;
          printf("\tMultiple chains started. Take 'multiple' default seed for parallel random number generator.\n");
        }
        break;
    ...
    if (seedBool==-1) {
      seedBool = 0;
      printf("Take default seed for random number generator.");
    }
    printf("\n");
  }
  ...
  if (color_split!=MPI_UNDEFINED) { // lasse die überflüssigen Prozessoren außer acht.
    ...
    if (chain_rank==masterrank) {
      switch (seedBool) {
        case 0: // falls nur eine Kette gestartet wird.
          gslrng = gsl_rng_alloc(gsl_rng_taus);
          break;
        case 1: // falls mehrere Ketten gestartet sind, und wir Reproduzierbarkeit brauchen
          // create an instance of a random number generator of type gsl_rng_sprng20
          gslrng = gsl_rng_alloc(gsl_rng_sprng20);
          break;
        case 2: // falls mehrere Ketten gestartet sind, und wir in jedem Testlauf anderes Verhalten wollen
          // create an instance of a random number generator of type gsl_rng_sprng20
          gslrng  = gsl_rng_alloc(gsl_rng_sprng20);
    ...
    strcat(filename_open,filename);
    filenIterSundials=fopen(filename_open,"w");
    ...
    */
    if (chain_rank==masterrank) {
      Ith(thetaCurr,1) = log(0.9);//log(0.4) + (world_rank - (int)round(world_size/2)) * 0.1; // sigma der Likelihood-Funktion
    } else {
      ...
      for (ctr = 0; ctr < blocksPerRank; ctr++) {
        for (kk = 1; kk <= 21; kk++) {
          ...
          Ith(thetaCurr,ctr*NPARSTUDY+kk) = Ith(posteriorZurlindenMean,kk);// + Ith(posteriorZurlindenMean,kk) * Ith(posteriorZurlindenCoV,kk); //(world_rank - (int)round(world_size/2)) * log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);
        }
        ...
        //     printf("%g * %g = %g\n",log(Ith(posteriorZurlindenMean,kk)),Ith(posteriorZurlindenCoV,kk),log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk));
        //   }
        // }
      }
    ...
    endtime_setup = MPI_Wtime();
    cputime_setup = endtime_setup - starttime_setup; fprintf(fileTimes, "setup\t%10.6e",cputime_setup);
    ...
    for (iterJ = 0; iterJ < iterAll; iterJ++) {
      ...
      if (chain_rank!=masterrank) {
        ...
        for (studyblock = chain_rank * blocksPerRank; studyblock < (chain_rank+1)*blocksPerRank; studyblock++) {
          ...
          for (kk = 1; kk <= NPARSTUDY; kk++) {
            ...
            // }
            if (iterJ>0) {
              // wenn Y~N(0,1) -> Z = sigma * Y + mu ist Z~N(mu,sigma^2). Ich ziehe auf Master eine N(0,1) Variable: sigmaRW_recv[ctr*(NPARSTUDY+1) + kk-1] .
              // Ith(thetaCan,kk) += SIGMA_RW * log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk) * sigmaRW_recv[ctr*(NPARSTUDY+1) + kk-1]; // Random Walk: auf die aktuelle Position einen Schritt gemäß einer Normalverteilung gehen.
              Ith(thetaCan,kk) = Ith(posteriorZurlindenMean,kk) + (Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);// ((((double)(iterAll-iterJ)/iterAll)*2.0)-1.0) * (Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);
              // Ith(thetaCan,kk) += sigmaRW_recv[ctr*(NPARSTUDY+1) + kk-1]; // Random Walk: auf die aktuelle Position einen Schritt gemäß einer Normalverteilung gehen.
            }
          ...
          if ((iterJ<iterAll-1)&&(studyblock==(chain_rank+1)*blocksPerRank-1)) {
            // Kommunikation erst beim letzten studyblock wieder anstoßen, sonst ist der buffer nicht sicher.
          ...
          data->T_G           = Ith(thetaCan,1);   // hour (h)
          data->T_P           = Ith(thetaCan,2);   // hour (h)
          data->K_APAP_Mcyp   = Ith(thetaCan,3);   // microM (\myM)
          data->V_MCcyp       = Ith(thetaCan,4);   // micromol per hour and per BW^{0.75} (\mymol/h/BW^{0.75})
          data->K_APAP_Msult  = Ith(thetaCan,5);   // microM (\myM)
          data->K_APAP_Isult  = Ith(thetaCan,6);   // microM (\myM)
          data->K_PAPS_Msult  = Ith(thetaCan,7);   // unitless
          data->V_MCsult      = Ith(thetaCan,8);   // microM per hour and per BW^{0.75} (\myM/h/BW^{0.75})
          data->K_APAP_Mugt   = Ith(thetaCan,9);   // microM (\myM)
          data->K_APAP_Iugt   = Ith(thetaCan,10);  // microM (\myM)
          data->K_UDPGA_Mugt  = Ith(thetaCan,11);  // unitless
          data->V_MCugt       = Ith(thetaCan,12);  // microM per hour and per BW^{0.75} (\myM/h/BW^{0.75})
          data->K_APAPG_Mmem  = Ith(thetaCan,13);  // microM (\myM)
          data->V_APAPG_Mmem  = Ith(thetaCan,14);  // micromol per hour (\mymol/h)
          data->K_APAPS_Mmem  = Ith(thetaCan,15);  // microM (\myM)
          data->V_APAPS_Mmem  = Ith(thetaCan,16);  // micromol per hour (\mymol/h)
          data->k_synUDPGA    = Ith(thetaCan,17);  // per hour (1/h)
          data->k_synPAPS     = Ith(thetaCan,18);  // per hour (1/h)
          data->k_APAP_R0     = Ith(thetaCan,19);  // liter per hour per BW^{0.75} (L/h/BW^{0.75})
          data->k_APAPG_R0    = Ith(thetaCan,20);  // liter per hour per BW^{0.75} (L/h/BW^{0.75})
          data->k_APAPS_R0    = Ith(thetaCan,21);  // liter per hour per BW^{0.75} (L/h/BW^{0.75})
        ...
        endtime_iter = MPI_Wtime();
        cputime_iter = endtime_iter - starttime_iter; fprintf(fileTimes, "\ncpuiter\t%10.6e", cputime_iter);
      } // if chain_rank!=masterrank
      else {
        /**************************************************************************/
        /**************************** Block: sigmaLL ******************************/
        ...
        if (iterJ>0) {
          ...
          sigmaLL_Can_next = gsl_ran_gaussian(gslrng,SIGMA_LL);
            ...
            if (chain_rank==masterrank) { // beachte nur die Kettenmaster
              if (world_rank==masterrank) { // eine Kette würfelt das Schema
                ...
                for (i = 0; i < swaps_max; i++) {
                  fprintf(fileSolution, "%d ", swapscheme[i]);
                }
        ...
        for (i = 0; i < NSTUDIES; i++) {
          ...
          for (k = 0; k < NPARSTUDY; k++) {
            param_Can = gsl_ran_gaussian(gslrng, 1.0); // schon bei iterJ==0 anfangen, damit die dann per MPI_Iscatterv möglichst nichtblockierend versendet werden
        ...
        if (iterJ>0) {
          ...
          // Ith(thetaCan,1) = log((double) 50 * (iterAll-iterJ)/iterAll);
        }
        ...
        logPrior_can = - 0.5*log(2*M_PI*1*exp(Ith(thetaCan,1))) - pow(Ith(thetaCan,1) + 32,2) / (2*1); // log( logNormal(-32,1) )

        /* posterior berechnen (LogPosterior) */
        logPosterior_can_sigma = logLikelihood_can + logPrior_can;
        // printf("%g \t+ %g \t= ",logLikelihood_can,logPrior_can);
        ...
        if (iterJ>0) { // bei j==0 wird noch der theta_Anfangswertvektor ausgewertet
          ...
          // printf("%g \t- %g \t= %g\t%g\n", logPosterior_can_sigma, logPosterior_curr_sigma,logaccrate,sigmaLL_AccRej_next);
          /************************************************************************/
          /****************** accept-reject Schritt *******************************/
          /************************************************************************/
          // logaccrateUniform = log(gsl_rng_uniform(gslrng));             // LogUniform, um mit LogPosterior vergleichen zu können!
          ...
          // fprintf(fileMarkovChain, "%d\t%d\t%14.6e\n", iterJ, 10, Ith(thetaCurr,1));
          logPosterior_curr_sigma = logPosterior_can_sigma; //printf("%g - %g = %g\n", logPosterior_can_sigma, logPosterior_curr_sigma,logPosterior_can_sigma-logPosterior_curr_sigma);
          nAcceptBool = 1;
        }
        fprintf(fileCandidates, "\t%d\n", nAcceptBool);
        /*************************** Block sigmaLL ENDE ***************************/
      ...
      // write thetaCurr to file
      if (chain_rank!=masterrank) {
        ctr = 0; // für passenden Zugriff auf aktuellem studyblock
        for (studyblock = chain_rank * blocksPerRank; studyblock < (chain_rank+1)*blocksPerRank; studyblock++) {
          fprintf(fileMarkovChain, "%d\t%d", iterJ, studyblock);
          for (kk = 1; kk <= NPARSTUDY; kk++) {
            // write thetaCurr to file
    ...
    if (chain_rank!=masterrank) {
      ...
      fprintf(fileLog, "\n");
      acceptrate = (double) acceptrate/iterAll/blocksPerRank;
    } else {
      fprintf(fileLog, "\t%d\n", (int)Ith(nAccepted,1));
