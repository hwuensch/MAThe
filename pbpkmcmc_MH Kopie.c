 ...
 * MH:
 *
 * Der grundsätzliche Metropolis-Hastings mit symmetrischem Random-Walk
 * als Proposalverteilung und je nach Übergabeparameter mehrkettig und/oder
 * mit Positionstausch.
 * -----------------------------------------------------------------
 * Input:
 *  - iterAll:  gibt die # an auszuführenden Iterationen an. (int)
 *  - swaps:    gibt an, zwischen wie vielen Ketten getauscht werden soll. (int)
 * -----------------------------------------------------------------
...
int main(int argc,char *argv[])
{
  ...
  double thetaCurr_swap_send[NPARALL], thetaCurr_swap_recv[NPARALL];
  realtype reltol, t, tout;
  N_Vector y_Sundials, abstol, thetaCurr, thetaCan, thetaCan_temp, odeSolution, posteriorZurlindenMean, posteriorZurlindenCoV;
  N_Vector data2fit, odeSolution2save;
  UserData data;
  SUNMatrix A;
  SUNLinearSolver LS;
  void *cvode_mem;
  int world_rank, world_size, tag=42, swapPartner;
  int retval, retval2, iout, i, ctr, iterJ, k, kk, iterAll, nData, nAPAP_G_S, studyblock, colsODESolu=3, rowODESolu=0, LogNormalBool=0;
  int rootsfound[2], logLikelihood_nan, ode_fail=0, nAccepted, waitbar, waitbar_counter, onepercent, nAcceptBool;
  ...
  realtype logPrior_can, logPrior_can_temp, logLikelihood_can, logLikelihood_can_temp, logPosterior, logPosterior_can, logPosterior_curr, logPosterior_swap;
  gsl_rng *gslrng;
  FILE *fileLog, *fileSolution, *fileMarkovChain, *fileAcceptReject, *fileCandidates, *fileTimes;
  ...
  sprintf(filename_dir,"output/MH/%d/%d_%d_",iterAll,world_size,swaps);
  ...
  thetaCan_temp = NULL; thetaCan_temp = N_VNew_Serial(NPARALL); if (check_retval((void *)thetaCan_temp, "N_VNew_Serial", 0)) return(1);
  ...
  odeSolution = NULL;
  odeSolution2save = NULL; odeSolution2save = N_VNew_Serial(393); if (check_retval((void *)odeSolution2save, "N_VNew_Serial", 0)) return(1);
  ...
  sigmaRW_scale = .05;                                                          // persönlicher Skalierungsfaktor
  ...
  */
  for (i = 0; i < NSTUDIES; i++) {
    ...
    Ith(thetaCurr,i*NPARSTUDY+1) = log(0.332) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);    // hour (h)
    Ith(thetaCurr,i*NPARSTUDY+2) = log(0.0476) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);   // hour (h)
    Ith(thetaCurr,i*NPARSTUDY+3) = log(123.0) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);    // microM (\myM)
    Ith(thetaCurr,i*NPARSTUDY+4) = log(2.57) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);     // micromol per hour and per BW^{0.75} (\mymol/h/BW^{0.75})
    Ith(thetaCurr,i*NPARSTUDY+5) = log(1.2e+3) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);   // microM (\myM)
    Ith(thetaCurr,i*NPARSTUDY+6) = log(478.0) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);    // microM (\myM)
    Ith(thetaCurr,i*NPARSTUDY+7) = log(0.345) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);    // unitless
    Ith(thetaCurr,i*NPARSTUDY+8) = log(467.0) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);    // microM per hour and per BW^{0.75} (\myM/h/BW^{0.75})
    Ith(thetaCurr,i*NPARSTUDY+9) = log(6.14e+3) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);  // microM (\myM)
    Ith(thetaCurr,i*NPARSTUDY+10) = log(4.99e+4) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);  // microM (\myM)
    Ith(thetaCurr,i*NPARSTUDY+11) = log(0.343) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);    // unitless
    Ith(thetaCurr,i*NPARSTUDY+12) = log(5.21e+3) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);  // microM per hour and per BW^{0.75} (\myM/h/BW^{0.75})
    Ith(thetaCurr,i*NPARSTUDY+13) = log(1.75e+4) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);  // microM (\myM)
    Ith(thetaCurr,i*NPARSTUDY+14) = log(3.54e+4) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);  // micromol per hour (\mymol/h)
    Ith(thetaCurr,i*NPARSTUDY+15) = log(2.23e+4) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);  // microM (\myM)
    Ith(thetaCurr,i*NPARSTUDY+16) = log(1.4e+7) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);   // micromol per hour (\mymol/h)
    Ith(thetaCurr,i*NPARSTUDY+17) = log(3.6e+4) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);   // per hour (1/h)
    Ith(thetaCurr,i*NPARSTUDY+18) = log(3.66e+3) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);  // per hour (1/h)
    Ith(thetaCurr,i*NPARSTUDY+19) = log(0.0123) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);   // liter per hour per BW^{0.75} (L/h/BW^{0.75})
    Ith(thetaCurr,i*NPARSTUDY+20) = log(0.155) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);    // liter per hour per BW^{0.75} (L/h/BW^{0.75})
    Ith(thetaCurr,i*NPARSTUDY+21) = log(0.138) + log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk);    // liter per hour per BW^{0.75} (L/h/BW^{0.75})
...
//  if (check_retval(&retval, "CVodeRootInit", 1)) return(1);
  ...
  /****************************************************************************/
  /****************************************************************************/
  nAccepted = 0; onepercent = iterAll/100; waitbar_counter = 0;
    ...
    */
    ...
    for (i = 0; i < NSTUDIES; i++) {
      for (kk = 1; kk <= NPARSTUDY; kk++) {
        Ith(thetaCan,i*NPARSTUDY+kk) = Ith(thetaCurr,i*NPARSTUDY+kk);
        if (iterJ>0) { // bei iterJ==0 den Startwertvektor als "Kandidatenvektor" nehmen, damit ich nicht alles doppelt schreiben muss.
          sigmaRW = sigmaRW_scale * log(Ith(posteriorZurlindenMean,kk)) * Ith(posteriorZurlindenCoV,kk); // mean logarithmieren, weil: Kap. 7.3 von https://www.staff.uni-giessen.de/~gi38/publica/pharma/kapitel7.html
          sigmaRW = 1e-1;
          Ith(thetaCan,i*NPARSTUDY+kk) += gsl_ran_gaussian(gslrng,sigmaRW);
        }
      }
    }
    // sigmaLL würfeln
    Ith(thetaCan,NPARALL) = Ith(thetaCurr,NPARALL)+gsl_ran_gaussian(gslrng,1e-3); // würfele sigmaLL nach logNormal(-1,1)

    endtime_proposal = MPI_Wtime(); cputime_proposal = endtime_proposal-starttime_proposal; // fprintf(fileTimes, "\nproposal\t%10.6e", cputime_proposal);
    /**************************************************************************/
    ...
    for (studyblock = 0; studyblock < NSTUDIES; studyblock++) {
      ...
      odeSolution = N_VNew_Serial(nData*nAPAP_G_S); if (check_retval((void *)odeSolution, "N_VNew_Serial", 0)) return(1); // der Lösungsvektor benötigt nicht den Datenzeitpunkt (im Vgl. zu data2fit)
      for (kk = 1; kk <= nData*nAPAP_G_S; kk++) Ith(odeSolution,kk) = 0.0; // init
      ...
      while(1) {
        ...
        if (check_retval(&retval, "CVode", 1)) {
          ...
          //   fprintf(fileLog,"%14.6e ", Ith(thetaCan,studyblock*NPARSTUDY+i));
          // }
          // fprintf(fileLog,"\n");
          nAcceptBool = -2;
          break;
        ...
        if (retval == CV_SUCCESS) {
          ...
          if (iterJ==0) {
            fprintf(fileSolution,"t=%0.4e\t", t);
            for (i = 1; i <= 3; i++) fprintf(fileSolution,"%14.6e ", Ith(y_Sundials,i*10)); fprintf(fileSolution, "\n");
          }

          // save current step of solution into vector (for likelihood)
          for (kk = 1; kk <= colsODESolu; kk++) {
            // Ith(odeSolution2save,rowODESolu*colsODESolu + iout*colsODESolu + kk) = Ith(y_Sundials,10*kk);
            if (kk<=nAPAP_G_S) {
              Ith(odeSolution,iout*nAPAP_G_S + kk) = Ith(y_Sundials,10*kk); // 10*kk sind die Ergebnisse bzgl. Venenkonzentration. (10, weil es jeweils 10 Kompartimente für APAP,-G,-S sind)
            }
          }
      ...
      retval = getLogLikelihood(nData, nAPAP_G_S, &logLikelihood_can_temp, odeSolution, data2fit, exp(Ith(thetaCan,NPARALL)), LogNormalBool);
      logLikelihood_can += logLikelihood_can_temp;
      ...
      retval = getLogPrior(&logPrior_can_temp, thetaCan_temp);
      logPrior_can += logPrior_can_temp;

      // fprintf(fileLog,"\nlogPrior_can_temp: %g \tlogLikelihood_can_temp : %g\n", logPrior_can_temp, logLikelihood_can_temp);
    ...
    // printf("iterJ %d: logLikelihood_can + logPrior_can = %g + %g = %g\n", iterJ, logLikelihood_can,logPrior_can, logLikelihood_can + logPrior_can);
    logPosterior_can = logLikelihood_can + logPrior_can;
    ...
    cputime_posterior_rest = endtime_posterior-endtime_ODE; // fprintf(fileTimes, "\nposteriorRest\t%10.6e", cputime_posterior_rest);
    cputime_posterior = endtime_posterior-endtime_proposal; fprintf(fileTimes, "\nposterior\t%10.6e", cputime_posterior);

    if (iterJ>0) { // bei j==0 wird noch der theta_Anfangswertvektor ausgewertet
      /************************************************************************/
      /****************** akzeptanzrate berechnen *****************************/
      /************************************************************************/
      // printf("logPosterior_can - logPosterior_curr = %g - %g\n", logPosterior_can,logPosterior_curr);
      logaccrate = logPosterior_can - logPosterior_curr;  // in dieser Form, solange Proposal symmetrisch ist! Sonst nochmal ändern!
      ...
      if(logaccrateUniform < logaccrate) { // accept
        ...
        for (i = 0; i < NSTUDIES; i++) {
          ...
          for (kk = 1; kk <= NPARSTUDY; kk++) {
            ...
            Ith(thetaCurr,i*NPARSTUDY+kk) = Ith(thetaCan,i*NPARSTUDY+kk);
            // write thetaCurr to file
            fprintf(fileMarkovChain, "\t%14.6e", Ith(thetaCurr,i*NPARSTUDY+kk));
          }
      ...
      if ((world_size>1)&&(swaps>1)) {
        ...
        if (world_rank!=swapPartner) {
          ...
          MPI_Isend(&logPosterior_curr, 1, MPI_DOUBLE, swapPartner, tag, MPI_COMM_WORLD, &request_swap_logPosterior);
          ...
          MPI_Recv(&logPosterior_swap, 1, MPI_DOUBLE, swapPartner, tag, MPI_COMM_WORLD, &status_swap_logPosterior);
          ...
          logPosterior_curr = logPosterior_swap;
        }
      } // if world_size > 1
    ...
    // write candidate into file
    for (i = 0; i < NSTUDIES; i++) {
      fprintf(fileCandidates, "%d\t%d", iterJ, i);
      for (kk = 1; kk <= NPARSTUDY; kk++) {
        fprintf(fileCandidates, "\t%14.6e", Ith(thetaCan,i*NPARSTUDY+kk));
      }
      fprintf(fileCandidates, "\t%d\n", nAcceptBool);
    }
    fprintf(fileCandidates, "%d\t%d\t%14.6e\t%d\n", iterJ, 10, Ith(thetaCan,NPARALL), nAcceptBool);

    nAcceptBool = 0;                                                            // zurücksetzen, falls es auf -2 war
  } // end of 'iterAll'
  printf("100%%\n\n");
  ...
  fprintf(fileTimes, "\nsecPerSundialsIter\t%ld\t%10.6e\t%10.6e", nIterSundials_sum, (double) cputime_ODE_sum/nIterSundials_sum, (double) cputime_ODE_fail/nIterSundials_fail);
  /****************************************************************************/
  /************************ end of main loop **********************************/
  /****************************************************************************/
  ...
  // acceptrate = (double) nAccepted/iterAll;
  // fprintf(fileLog,"world_rank_%d:\taccepted:\t%d\taccrate:\t%g\n", world_rank, nAccepted, acceptrate);
