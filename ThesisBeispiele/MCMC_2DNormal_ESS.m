function ESS = MCMC_2DNormal_ESS(Kette,iterAll,dimension)
%% nach Schmidl et. al, 2013, S. 9
% 
% Kette kann mehrdimensional sein:
% Kette = dimension X iterAll+1,
%   d.h. die Kettenglieder sind Spalte für Spalte sortiert
...
for tau = 1:iterAll % lag \tau
    autocorrfct_est(zugriff,tau) = MCMC_2DNormal_AutocorrEst(Kette(zugriff,:),iterAll,dimension-nBreak,MW_est(zugriff,1),Var_est(zugriff,1),tau);
    p1 = (1 - tau/(iterAll+1)) .* autocorrfct_est(zugriff,tau);
    ESS_dim(zugriff,1) = ESS_dim(zugriff,1) + p1;
    
    argmin = autocorrfct_est(zugriff,tau) < 0.05; % die 0.05 sind willkürlich von Hoffman and Gelman, 2011, Anhang A gewählt
    if any(argmin) 
