...
function [theta_curr,theta_can] = MCMC_2DNormalblockweise1(iterAll,dimension,proposal_type,sigma_proposal,sigma_post,startpunkt,blocksize)
...
for b = 1:(length(block)-1)
    if b<dimension
        idx_block = block(b):(block(b+1)-1) ~= block;
        idx_block = block(idx_block);
        idx_block = idx_block(1:end-1);
    else
        idx_block = block(1:end-2);
    end
    mu = 0.9 * theta_curr(idx_block,1);
    posterior_curr(1,b)  = MCMC_2DNormal_Posterior(theta_curr(block(b):(block(b+1)-1),1),mu,1,0.19,blocksize);
    [~,q_curr(1,b)]      = MCMC_2DNormal_Proposal(mu,proposal_type_i,sigma_proposal,blocksize);
end
accept = 0;
...
for iterJ = 1:iterAll
    for b = 1:(length(block)-1)
        ...
        mu = 0.9 * theta_curr(idx_block,iterJ);
        [theta_can(block(b):(block(b+1)-1),iterJ), q_can] = ...
            MCMC_2DNormal_Proposal(mu,proposal_type_i,sigma_proposal,length(idx_block));
        %%% posterior of candidate
        posterior_can = MCMC_2DNormal_Posterior(theta_can(block(b):(block(b+1)-1),iterJ),mu,1,0.19,blocksize);
        %%% accept-reject
