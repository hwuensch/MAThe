...
#define T1    RCONST(1.0e-1)  /* first output time (1min = 0.0166...)     */
#define DTOUT RCONST(1.0e-1)
...
int main(int argc,char *argv[])
{
  ...
  gsl_rng *gslrng;
  FILE *fileLog, *fileSolution, *fileMarkovChain, *fileAcceptReject, *fileCandidates, *fileTimes, *fileCVode, *fileInvestigate;
  ...
  sprintf(filename,"investigate_%d.txt",world_rank);
  strcat(filename_open,filename);
  fileInvestigate=fopen(filename_open,"a"); // append to file
  if (fileInvestigate==NULL) {
    fileInvestigate = fopen(filename,"w");
  }
  if (fileInvestigate==NULL) {
    perror("Failed open");
    MPI_Finalize();
  ...
  sprintf(filename,"CVodeInfos_%d.txt",world_rank);
  strcat(filename_open,filename);
  fileCVode=fopen(filename_open,"a"); // append to file
  if (fileCVode==NULL) {
    fileCVode = fopen(filename,"w");
  }
  if (fileCVode==NULL) {
    perror("Failed open");
    MPI_Finalize();
...
//  if (check_retval(&retval, "CVodeRootInit", 1)) return(1);
  ...
  for (iterJ = 0; iterJ < iterAll; iterJ++) {
      ...
      */
      ...
      for (kk = 1; kk <= NPARSTUDY; kk++) {
        ...
        if (iterJ>0) { // bei iterJ==0 den Startwertvektor als "Kandidatenvektor" nehmen, damit ich nicht alles doppelt schreiben muss.
          ...
          Ith(thetaCan,studyblock*NPARSTUDY+kk) += gsl_ran_gaussian(gslrng,sigmaRW);
        }
      ...
      while(1) {
        ...
        if ((iterJ==288)&&(studyblock==4)&&(tout>9.9)) {
          tout +=1.0e-8;
          retval = CVode(cvode_mem, tout, y_Sundials, &t, CV_ONE_STEP);  // Löse bis zum Zeitpunkt tout
        } else {
          tout += DTOUT;
        ...
        if (iterJ==288) {
          retval2 = CVodeGetIntegratorStats(cvode_mem, &nsteps, &nfevals, &nlinsetups, &netfails, &qlast, &qcur, &hinused, &hlast, &hcur, &tcur);
          fprintf(fileCVode,"%d\t%d\t%g\t%ld\t%ld\t%ld\t%ld\t%d\t%d\t%g\t%g\t%g\t%g\n", iterJ, studyblock, tout, nsteps, nfevals, nlinsetups, netfails, qlast, qcur, hinused, hlast, hcur, tcur);
        }
        ...
        // if (iout == nData) break;
        if (tout > Ith(data2fit,(nData-1)*(nAPAP_G_S+1)+1)) break;
      }
      ...
      retval = getLogLikelihoodTheta(nData, nAPAP_G_S, &logLikelihood_can_blockj, &LikelihoodNumerator_blockj, odeSolution, data2fit, exp(Ith(thetaCan,NPARALL)), LogNormalBool);
      if (iterJ==0) {
      ...
      Ith(logPosterior_can,studyblock+1) = logLikelihood_can + logPrior_can;    // logPosterior_can als Vektor, um bei Akzeptanzrate mit dem passenden Block aus vorherigem Schritt vergleichen zu können
    ...
    if (iterJ>0){
      ...
      Ith(thetaCan,NPARALL) += gsl_ran_gaussian(gslrng,1); // würfele sigmaLL mittels Random Walk
    }
