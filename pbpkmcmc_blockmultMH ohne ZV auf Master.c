...
int main(int argc,char *argv[])
{
  ...
  double endtime_bcast_sigmaLL, endtime_gather_LLNumer;
  ...
  realtype logPrior_can, logPrior_can_temp, logLikelihood_can, logLikelihood_can_blockj, logPosterior, LikelihoodNumerator_blockj, sigmaLL;
  ...
  MPI_Request request_bcast, request_swap_logPosterior, request_swap_theta;
    ...
    */
    ...
    for (iterJ = 0; iterJ < iterAll; iterJ++) {
      ...
      if (chain_rank!=masterrank) {
        ...
        MPI_Ibcast(&sigmaLL,1,MPI_DOUBLE,masterrank,chain_comm,&request_bcast); // recv current sigmaLL for LikelihoodTheta function.
        ...
        for (studyblock = chain_rank * blocksPerRank; studyblock < (chain_rank+1)*blocksPerRank; studyblock++) {
          ...
          retval = getLogLikelihoodTheta(nData, nAPAP_G_S, &logLikelihood_can_blockj, &LikelihoodNumerator_blockj, odeSolution, data2fit, exp(sigmaLL), LogNormalBool);
          logLikelihood_can = logLikelihood_can_blockj;

          // MPI_Gather(); // send LikelihoodNumerator_blockj an masterrank
          // printf("%d: chain_rank %d before MPI_Send\t- studyblock %d\n", iterJ, chain_rank, studyblock);
          MPI_Send(&LikelihoodNumerator_blockj,1,MPI_DOUBLE,masterrank,tag,chain_comm); // send LikelihoodNumerator_blockj an masterrank
          endtime_gather_LLNumer = MPI_Wtime(); cputime_gather_LLNumer = endtime_gather_LLNumer - endtime_bcast_sigmaLL; fprintf(fileTimes, "\nposteriorGatherLLNumer\t%d\t%10.6e", studyblock, cputime_gather_LLNumer);
          cputime_gather_LLNumer_sum += cputime_gather_LLNumer;
          // printf("%d: chain_rank %d after MPI_Send\t- studyblock %d to %d\n", iterJ, chain_rank, studyblock, masterrank);
        ...
        endtime_proposal = MPI_Wtime(); cputime_proposal = endtime_proposal - starttime_proposal; studyblock=RANKMASTER; // fprintf(fileTimes, "\nproposal\t%d\t%10.6e", studyblock, cputime_proposal);
        ...
        MPI_Ibcast(&sigmaLL,1,MPI_DOUBLE,masterrank,chain_comm,&request_bcast); // initiate a broadcast relating to sigmaLL
        // for (i = 0; i < chain_size-1; i++) {
        //   // printf("send %g to %d\n", sigmaLL, i);
        //   MPI_Send(&sigmaLL,1,MPI_DOUBLE,i,tag,chain_comm);
        //   // printf("send to %d finished\n",i);
        // }
        ...
        for (i = 0; i < nClients; i++) { // nClients
          for (studyblock = i * blocksPerRank; studyblock < (i+1)*blocksPerRank; studyblock++) {
            ...
            /************************* löse ODE System ****************************/
            /* likelihood berechnen (als LogLikelihood) */
            // printf("%d: masterrank %d before MPI_Recv\t- studyblock %d\n", iterJ, chain_rank, studyblock);
            MPI_Recv(&LikelihoodNumerator_blockj,1,MPI_DOUBLE,i,tag,chain_comm,&status);
            endtime_gather_LLNumer = MPI_Wtime(); cputime_gather_LLNumer = endtime_gather_LLNumer - endtime_getInfos; fprintf(fileTimes, "\nposteriorGatherLLNumer\t%d\t%10.6e", studyblock, cputime_gather_LLNumer);
            cputime_gather_LLNumer_sum += cputime_gather_LLNumer;
            // printf("%d: masterrank %d after MPI_Recv\t- studyblock %d\n", iterJ, chain_rank, studyblock);

            Ith(LikelihoodNumerator,studyblock+1) = LikelihoodNumerator_blockj;
