...
#define SIGMA_RW RCONST(1e-4)
#define SIGMA_LL RCONST(1e-2)
...
int main(int argc,char *argv[])
{
  ...
  if (color_split!=MPI_UNDEFINED) { // lasse die überflüssigen Prozessoren außer acht.
    ...
    // write into log-file
    fprintf(fileLog, "SIGMA_RW = %g\tSIGMA_LL = %g\n",SIGMA_RW,SIGMA_LL);
    ...
    */
    if (chain_rank==masterrank) {
      Ith(thetaCurr,1) = log(0.9);// + (world_rank - (int)round(world_size/2)) * 0.1; // sigma der Likelihood-Funktion
    } else {
      ...
      for (ctr = 0; ctr < blocksPerRank; ctr++) {
        for (kk = 1; kk <= 21; kk++) {
          ...
          Ith(thetaCurr,ctr*NPARSTUDY+kk) = Ith(posteriorZurlindenMean,kk) + Ith(posteriorZurlindenMean,kk) * Ith(posteriorZurlindenCoV,kk);// (world_rank - (int)round(world_size/2)) * Ith(posteriorZurlindenMean,kk) * Ith(posteriorZurlindenCoV,kk);
        }
    ...
    for (iterJ = 0; iterJ < iterAll; iterJ++) {
      ...
      if (chain_rank!=masterrank) {
        ...
        for (studyblock = chain_rank * blocksPerRank; studyblock < (chain_rank+1)*blocksPerRank; studyblock++) {
          ...
          for (kk = 1; kk <= NPARSTUDY; kk++) {
            ...
            if (iterJ>0) {
              ...
              Ith(thetaCan,kk) += SIGMA_RW * Ith(posteriorZurlindenMean,kk) * Ith(posteriorZurlindenCoV,kk) * sigmaRW_recv[ctr*(NPARSTUDY+1) + kk-1]; // Random Walk: auf die aktuelle Position einen Schritt gemäß einer Normalverteilung gehen.
        ...
        if (iterJ>0) {
          ...
          // Ith(thetaCan,1) += gsl_ran_gaussian(gslrng,SIGMA_LL); // würfele sigmaLL mittels Random Walk
